...
Composite action: `FounderB/BucketScanner/action@v1.9.2` (or `./action` from a checkout).
...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: FounderB/BucketScanner/action@v1.9.2
        with:
          profile: yc-prod
...
|-------|---------|-------------|
| `version` | — | Install `bucket-scanner==<version>` from PyPI (e.g. `1.9.2`) |
...
| `run-doctor` | `false` | Run `bucket-scanner doctor --json` before scan |
| `doctor-only` | `false` | Health check only (skip scan) |
